(function(){
 ...
 YP.questionRowToClient=function(row,fallback={}){
   ...
   const originalChoice=YP.normalizeChoiceChallenge?YP.normalizeChoiceChallenge(original):null,similarChoice=YP.normalizeChoiceChallenge?YP.normalizeChoiceChallenge(similar):null;
   if(originalChoice?.valid)q.originalRetry=originalChoice;
   if(similarChoice?.valid)q.similarProblem=similarChoice;
 ...
 YP.mergeExamQuestions=function(exam,rows){
   const out=YP.clone(exam);if(!out||!Array.isArray(out.questions))return out;
   ...
   out.questions=out.questions.map(q=>{
     const original=YP.normalizeChoiceChallenge(q.originalRetry),similar=YP.normalizeChoiceChallenge(q.similarProblem);
     return {...q,originalRetry:original,similarProblem:similar,retryChoiceReady:original.valid&&similar.valid};
   });
 ...
 YP.normalizeChoiceChallenge=function(config){
   const source=YP.clone(config||{}),rawChoices=Array.isArray(source.choices)?source.choices:[],choices=rawChoices.map(x=>String(x??"").trim()),correctChoice=Number(source.correctChoice||source.answerIndex||0),canonical=choices.map(x=>x.toLowerCase().replace(/\s+/g," "));
   let message="";
   if(![4,5].includes(choices.length))message="객관식 보기는 4개 또는 5개로 구성되어야 합니다.";
   else if(choices.some(x=>!x))message="빈 보기가 있어 재구성이 필요합니다.";
   else if(new Set(canonical).size!==choices.length)message="중복된 보기가 있어 재구성이 필요합니다.";
   else if(!Number.isInteger(correctChoice)||correctChoice<1||correctChoice>choices.length)message="정답 보기 번호가 올바르지 않습니다.";
   const choiceAnswer=choices[correctChoice-1]||"";
   return {...source,inputMode:"choice",choices,correctChoice,answer:String(source.answer||choiceAnswer),acceptableAnswers:[String(correctChoice),choiceAnswer].filter(Boolean),valid:!message,message,choiceUnavailable:!!message};
 };
 YP.checkChoiceAnswer=function(input,config){const normalized=YP.normalizeChoiceChallenge(config),value=Number(String(input??"").trim());return normalized.valid&&Number.isInteger(value)&&value>0?value===normalized.correctChoice:null};
 ...
 /** 한 학생의 여러 시험 기록을 누적 분석한다. 서버 포털과 데모 모드가 같은 계산 규칙을 사용한다. */
 YP.computeStudentPortalCumulative=function(records){
   const normalized=(records||[]).map(YP.normalizeRecord).filter(r=>YP.getExam(r.examId)&&Number.isFinite(Number(r.percent))),unitMap=new Map(),courseMap=new Map(),weakItems=[];let totalScore=0,totalMax=0,percentSum=0,full=0,partial=0,wrong=0,ungraded=0;
   const reports=normalized.map(r=>{const ex=YP.getExam(r.examId),counts=r.counts||{full:0,partial:0,wrong:0,ungraded:0},snapshot=new Map();totalScore+=Number(r.score||0);totalMax+=Number(r.maxScore||0);percentSum+=Number(r.percent||0);full+=Number(counts.full||0);partial+=Number(counts.partial||0);wrong+=Number(counts.wrong||0);ungraded+=Number(counts.ungraded||0);(ex.questions||[]).forEach((q,i)=>{const s=r.scoring?.[i]||{},key=[r.courseId,q.unit].join("|"),u=unitMap.get(key)||{courseId:r.courseId,unit:q.unit,score:0,maxPoints:0,questionCount:0,wrongCount:0,partialCount:0};u.score+=Number(s.score||0);u.maxPoints+=Number(q.maxPoints||0);u.questionCount++;if(s.status==="wrong")u.wrongCount++;if(s.status==="partial")u.partialCount++;unitMap.set(key,u);const su=snapshot.get(q.unit)||{unit:q.unit,score:0,maxPoints:0};su.score+=Number(s.score||0);su.maxPoints+=Number(q.maxPoints||0);snapshot.set(q.unit,su);if(["wrong","partial"].includes(s.status))weakItems.push({reportToken:r.token,examId:r.examId,courseId:r.courseId,questionNo:q.no,unit:q.unit,topic:q.topic,status:s.status,score:s.score,maxPoints:q.maxPoints})});const c=courseMap.get(r.courseId)||{courseId:r.courseId,score:0,maxPoints:0,count:0};c.score+=Number(r.score||0);c.maxPoints+=Number(r.maxScore||0);c.count++;courseMap.set(r.courseId,c);return {reportToken:r.token,examId:r.examId,courseId:r.courseId,title:ex.title,shortTitle:ex.shortTitle||ex.title,assessmentType:ex.assessmentType||"weekly",section:ex.section||"",round:Number(ex.round||0),score:r.score,maxScore:r.maxScore,percent:r.percent,counts,unitSnapshot:[...snapshot.values()].map(u=>({...u,percent:u.maxPoints?u.score/u.maxPoints*100:0})),createdAt:r.createdAt,updatedAt:r.updatedAt}});
   const units=[...unitMap.values()].map(u=>({...u,percent:u.maxPoints?u.score/u.maxPoints*100:0})).sort((a,b)=>b.percent-a.percent),courses=[...courseMap.values()].map(c=>({...c,percent:c.maxPoints?c.score/c.maxPoints*100:0})).sort((a,b)=>b.percent-a.percent),chronological=[...reports].sort((a,b)=>String(a.updatedAt||a.createdAt).localeCompare(String(b.updatedAt||b.createdAt)));
   return {reports:[...reports].sort((a,b)=>String(b.updatedAt||b.createdAt).localeCompare(String(a.updatedAt||a.createdAt))),cumulative:{testCount:reports.length,weeklyCount:reports.filter(x=>x.assessmentType!=="comprehensive").length,comprehensiveCount:reports.filter(x=>x.assessmentType==="comprehensive").length,totalScore,totalMaxScore:totalMax,weightedPercent:totalMax?totalScore/totalMax*100:0,averagePercent:reports.length?percentSum/reports.length:0,latestPercent:chronological.length?chronological.at(-1).percent:0,counts:{full,partial,wrong,ungraded},units,courses,trend:chronological.map(x=>({reportToken:x.reportToken,examId:x.examId,label:x.shortTitle,percent:x.percent,assessmentType:x.assessmentType,updatedAt:x.updatedAt})),strongUnits:units.slice(0,3),weakUnits:[...units].sort((a,b)=>a.percent-b.percent).slice(0,3),weakItems}};
 };
 ...
 YP.checkSimilarAnswer=function(input,similar){const normalized=YP.normalizeChoiceChallenge(similar);return {mode:"choice",correct:normalized.valid?YP.checkChoiceAnswer(input,normalized):null,unavailable:!normalized.valid}};
