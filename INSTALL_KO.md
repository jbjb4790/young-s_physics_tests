# v3.5.0 업데이트 — 학생 한 명당 하나의 영구 학부모 통합 링크

1. 운영 Spreadsheet를 백업합니다.
2. `apps-script/Code.gs`를 v3.5.0으로 전체 교체하고 기존 웹 앱을 **새 버전**으로 재배포합니다. `appsscript.json` 권한은 바뀌지 않습니다.
3. `installYoungsPhysics()`, `resetTeacherPin()`, `repairReportStorage()`는 실행하지 않습니다.
4. Apps Script 함수 목록에서 `migrateStudentPortals()`를 실행합니다. 결과의 `done`이 `false`이면 다시 실행하여 `done:true`가 될 때까지 반복합니다. 기존 점수·시험별 토큰·학부모 링크는 유지됩니다.
5. GitHub v3.5.0 기능 패치를 덮어씁니다. 운영 중인 `.github/workflows/pages.yml`과 `site/assets/runtime-config.js`는 패치에 포함되지 않으므로 기존 연결 설정을 유지합니다.
6. Pages 배포 후 강력 새로고침하고, 교사용 화면의 **학생 통합 링크** 표에서 학생별 영구 링크를 복사합니다.
7. 새 시험을 같은 학생 프로필로 저장하면 그 링크에 자동으로 누적되는지 확인합니다.

학생 식별은 우선 `StudentId`와 선택된 기존 프로필을 사용하고, 신규 저장에서는 외부 학생 ID 또는 학교·이름·학년·반번호를 활용합니다. 동일 학교·동일 이름의 후보가 여러 명이면 저장을 차단하고 기존 학생 선택 또는 동명이인 신규 학생 생성을 요구합니다.
